...
function DietaryBadge({ flag }: { flag: DietaryFlag }) {
  ...
  return (
    <View style={[staticStyles.dietaryBadge, { backgroundColor: config.color + '20' }]}>
      <Text style={[staticStyles.dietaryBadgeText, { color: config.color }]}>{config.label}</Text>
    </View>
  );
}
    ...
    <View style={styles.menuItem}>
      <View style={styles.menuItemHeader}>
        <View style={styles.menuItemNameRow}>
          ...
          {item.is_featured && (
            <View style={[staticStyles.featuredBadge, { backgroundColor: colors.accent + '20' }]}>
      ...
      {item.dietary_flags && item.dietary_flags.length > 0 && (
        <View style={staticStyles.dietaryFlags}>
...
const useMenuStyles = createLazyStyles((colors) => ({
  mainContainer: {
    flex: 1,
    minHeight: 400,
  },
