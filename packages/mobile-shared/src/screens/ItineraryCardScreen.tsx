 ...
 * Instagram Stories. Shows curated stops with images, walk times,
 * and branding. Users screenshot to share -- no native capture
 * modules needed (OTA-compatible).
 */
...
// Sequential display times per mood+slot
...
export default function ItineraryCardScreen() {
  ...
  const moodTitle = mood ? MOOD_TITLES[mood as ItineraryMood] : `YOUR ${brand.cityName.toUpperCase()} DAY`;
  ...
  const handleShare = useCallback(async () => {
    ...
    const moodLabel = mood ? ITINERARY_MOODS[mood as ItineraryMood].label : 'Day Out';

    let msg = `\uD83D\uDCCD ${moodLabel} in ${brand.cityName} \u2014 ${formattedDate}\n\n`;

    const timeLabels = mood ? MOOD_TIME_LABELS[mood as ItineraryMood] : DEFAULT_TIME_LABELS;

    items.forEach((item, i) => {
      const slot = item.time_slot as TimeSlot;
      const defaultSlot = SLOT_DISPLAY[slot];
      if (!defaultSlot) return;
      const override = mood ? MOOD_SLOT_OVERRIDES[mood as ItineraryMood]?.[slot] : undefined;
  ...
  }, [items, mood, formattedDate, walkMinutes, brand]);
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      await saveItineraryMutation.mutateAsync({
        itinerary: {
          title: `${brand.cityName} ${mood ? ITINERARY_MOODS[mood as ItineraryMood].label : 'Day'} \u2014 ${formattedDate}`,
          date,
          is_generated: true,
        },
        items,
  ...
  }, [items, mood, date, formattedDate, saveItineraryMutation, brand]);
  ...
  const handleShareToMove = useCallback(async () => {
    ...
    setIsSharing(true);
    try {
      // Save (or re-save) the itinerary to get a Supabase ID
  ...
  }, [items, mood, date, formattedDate, saveItineraryMutation, queryClient, brand]);
        ...
        <View style={styles.cardContainer}>
          ...
          <View style={styles.stopsContainer}>
            {items.map((item, index) => {
              ...
              const slotOverride = mood ? MOOD_SLOT_OVERRIDES[mood as ItineraryMood]?.[slot] : undefined;
                  ...
                  >
                    ...
                    <View style={styles.stopInfo}>
                      ...
                      <Text style={styles.stopTime}>
                        {(mood ? MOOD_TIME_LABELS[mood as ItineraryMood] : DEFAULT_TIME_LABELS)[slot] || slotLabel}
                      </Text>
                      {item.reason ? (
