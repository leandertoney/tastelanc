...
function BlogListItem({ post, onPress, aiName }: { post: BlogPost; onPress: () => void; aiName: string }) {
  ...
  return (
    <TouchableOpacity style={styles.listItem} onPress={onPress} activeOpacity={0.8}>
      ...
      <View style={styles.itemContent}>
        ...
        <Text style={styles.itemDate}>By {aiName} &middot; {formatDate(post.published_at || post.created_at)}</Text>
      </View>
    </TouchableOpacity>
  );
}
  ...
  return (
    <SafeAreaView style={styles.container} edges={['bottom']}>
      <FlatList
        data={filteredPosts}
        renderItem={({ item }) => (
          <BlogListItem post={item} onPress={() => handlePostPress(item)} aiName={brand.aiName} />
        )}
