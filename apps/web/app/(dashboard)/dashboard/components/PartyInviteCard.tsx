...
import { Copy, Check, Calendar, MapPin, ChevronDown, ChevronUp } from 'lucide-react';
...
export function PartyInviteCard({ restaurantId, buildApiUrl }: {
  ...
  return (
    <div className="rounded-xl overflow-hidden border border-[#C84B31]/40 bg-gradient-to-br from-gray-900 via-gray-900 to-[#C84B31]/5">
      {/* Top banner - clickable to expand/collapse */}
      <button
        onClick={() => setIsExpanded(!isExpanded)}
        className="w-full bg-[#C84B31] px-5 py-2.5 flex items-center gap-2 hover:bg-[#b03e27] transition-colors"
      >
        <span className="text-white font-semibold text-sm">🎉 Industry Party</span>
        <span className="text-[#ff9d87] text-xs">{eventDate}</span>
        {isExpanded ? (
          <ChevronUp className="w-4 h-4 text-white ml-auto" />
        ) : (
