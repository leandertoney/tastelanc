    ...
    <>
      ...
      <main className="max-w-4xl mx-auto px-4 py-10 text-tastelanc-text-primary">
        ...
        <div className="mt-4 p-4 bg-tastelanc-surface rounded-lg space-y-2">
          ...
          {event.description && <p className="text-tastelanc-text-secondary text-sm">{event.description}</p>}
          {event.cover_charge && <p className="text-sm text-tastelanc-text-secondary">Cover: ${event.cover_charge.toFixed(2)}</p>}
        </div>
      </main>
    </>
  );
