...
export default async function CategoriesPage() {
  ...
  if (!categories.length) return <main className="p-8 text-tastelanc-text-primary">No categories found.</main>;
  ...
  return (
    <>
      ...
      <main className="max-w-5xl mx-auto px-4 py-10 text-tastelanc-text-primary">
        ...
        <p className="text-tastelanc-text-muted mt-2">Explore restaurants by cuisine and style.</p>
        {restaurantCTAButtons()}
        <div className="grid md:grid-cols-2 gap-4 mt-6">
          {categories.map((c) => (
            <a key={c} href={`/categories/${slugify(c)}`} className="p-3 bg-tastelanc-surface rounded-lg block">
              <h2 className="text-lg font-semibold text-tastelanc-text-primary">{c}</h2>
            </a>
          ))}
        </div>
      </main>
