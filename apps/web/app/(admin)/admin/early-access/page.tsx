  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-xl md:text-2xl font-bold text-tastelanc-text-primary">Early Access Analytics</h1>
        ...
        {todayStats.map((stat) => (
          <div key={stat.label} className="bg-tastelanc-surface rounded-lg p-3 md:p-4 flex items-center justify-between">
            <div className="flex items-center gap-2 md:gap-3">
              <stat.icon className={`w-4 h-4 md:w-5 md:h-5 ${stat.color}`} />
              <span className="text-tastelanc-text-muted text-xs md:text-base">{stat.label}</span>
            </div>
            <span className="text-lg md:text-2xl font-bold text-tastelanc-text-primary">{stat.value}</span>
          </div>
        ))}
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-4 md:gap-6 mb-6 md:mb-8">
        ...
        <div className="bg-tastelanc-surface rounded-lg p-4 md:p-6">
          <h3 className="text-base md:text-lg font-semibold text-tastelanc-text-primary mb-3 md:mb-4 flex items-center gap-2">
            ...
            {data?.signupsByDay && Object.keys(data.signupsByDay).length > 0 ? (
              Object.entries(data.signupsByDay).map(([day, count]) => (
                <div key={day} className="flex items-center justify-between py-2 border-b border-tastelanc-surface-light last:border-0">
                  <span className="text-tastelanc-text-muted text-sm">{day}</span>
                  <span className="text-tastelanc-text-primary font-semibold">{count}</span>
                </div>
              ))
            ) : (
              <p className="text-tastelanc-text-faint text-sm">No signups yet</p>
            )}
          </div>
        </div>
        ...
        <div className="bg-tastelanc-surface rounded-lg p-4 md:p-6">
          ...
          <div className="space-y-2">
            {data?.viewsByDay && Object.keys(data.viewsByDay).length > 0 ? (
              ...
              <p className="text-tastelanc-text-faint text-sm">No page views yet</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="bg-tastelanc-surface rounded-lg p-4 md:p-6">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-tastelanc-surface-light">
                <th className="text-left py-3 px-4 text-tastelanc-text-muted font-medium">Email</th>
                <th className="text-left py-3 px-4 text-tastelanc-text-muted font-medium">Source</th>
                <th className="text-left py-3 px-4 text-tastelanc-text-muted font-medium">Signed Up</th>
                <th className="text-left py-3 px-4 text-tastelanc-text-muted font-medium">Status</th>
              </tr>
            </thead>
            <tbody>
              {data?.signups && data.signups.length > 0 ? (
                data.signups.map((signup) => (
                  <tr key={signup.id} className="border-b border-tastelanc-surface-light last:border-0">
                    <td className="py-3 px-4 text-tastelanc-text-primary">{signup.email}</td>
                    <td className="py-3 px-4 text-tastelanc-text-muted">{signup.source || 'premium_page'}</td>
                    <td className="py-3 px-4 text-tastelanc-text-muted">
                    ...
                    <td className="py-3 px-4">
                      {signup.converted_at ? (
                        ...
                        <span className="inline-flex items-center gap-1 text-tastelanc-text-faint text-sm">
                ...
                <tr>
                  <td colSpan={4} className="py-8 text-center text-tastelanc-text-faint">
        ...
        <div className="md:hidden space-y-3">
          {data?.signups && data.signups.length > 0 ? (
            data.signups.map((signup) => (
              <div key={signup.id} className="bg-tastelanc-surface-light/50 rounded-lg p-3">
                <div className="flex items-start justify-between gap-2 mb-2">
                  <p className="text-tastelanc-text-primary text-sm font-medium truncate flex-1">{signup.email}</p>
                  {signup.converted_at ? (
                    ...
                    <span className="inline-flex items-center gap-1 text-tastelanc-text-faint text-xs whitespace-nowrap">
                      <Clock className="w-3 h-3" />
                      Pending
                    </span>
                  )}
                </div>
                <div className="flex items-center justify-between text-xs text-tastelanc-text-muted">
            ...
            <p className="py-8 text-center text-tastelanc-text-faint text-sm">
              No signups yet. Share your early access page!
            </p>
          )}
        </div>
      ...
      <div className="mt-6 md:mt-8 bg-tastelanc-accent/10 border border-tastelanc-accent/30 rounded-lg p-3 md:p-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-tastelanc-text-primary font-medium text-sm md:text-base">Early Access Ends</p>
            <p className="text-tastelanc-text-muted text-xs md:text-sm">Dec 12th, 2025 at 11:59 PM EST</p>
          </div>
        </div>
      </div>
    </div>
