    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-tastelanc-surface rounded-xl border border-tastelanc-surface-light p-4">
          ...
          <p className="text-xs text-tastelanc-text-faint">Total Listings</p>
        </div>
        <div className="bg-tastelanc-surface rounded-xl border border-tastelanc-surface-light p-4">
          <p className="text-2xl font-bold text-blue-400">{stats.draft}</p>
          <p className="text-xs text-tastelanc-text-faint">Drafts</p>
        </div>
        <div className="bg-tastelanc-surface rounded-xl border border-tastelanc-surface-light p-4">
          <p className="text-2xl font-bold text-green-400">{stats.approved}</p>
          <p className="text-xs text-tastelanc-text-faint">Approved</p>
        </div>
        <div className="bg-tastelanc-surface rounded-xl border border-tastelanc-surface-light p-4">
          <p className="text-2xl font-bold text-emerald-400">{stats.posted}</p>
          <p className="text-xs text-tastelanc-text-faint">Posted</p>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-tastelanc-text-faint" />
          <input
            type="text"
            placeholder="Search listings..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full pl-10 pr-4 py-2 bg-tastelanc-surface-light border border-tastelanc-surface-light rounded-lg text-tastelanc-text-primary placeholder-tastelanc-text-faint focus:outline-none focus:border-tastelanc-accent"
          />
        </div>
        <select
          value={statusFilter}
          onChange={(e) => setStatusFilter(e.target.value)}
          className="px-3 py-2 bg-tastelanc-surface-light border border-tastelanc-surface-light rounded-lg text-tastelanc-text-primary focus:outline-none focus:border-tastelanc-accent"
        >
      ...
      {filteredJobs.length === 0 ? (
        <div className="bg-tastelanc-surface rounded-xl border border-tastelanc-surface-light p-12 text-center">
          <Briefcase className="w-12 h-12 text-tastelanc-text-faint mx-auto mb-3" />
          <p className="text-tastelanc-text-muted">No job listings found.</p>
        </div>
      ) : (
                ...
                <div className="flex flex-col md:flex-row md:items-start gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-1">
                      <h3 className="font-semibold text-tastelanc-text-primary">{job.title}</h3>
                      <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${status.className}`}>
                        {status.label}
                      </span>
                      <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-tastelanc-surface-light text-tastelanc-text-secondary">
                        {ROLE_LABELS[job.role_type] || job.role_type}
                      </span>
                    </div>

                    {job.expansion_cities && (
                      <p className="text-sm text-tastelanc-text-muted flex items-center gap-1 mb-2">
                    ...
                    {job.compensation_summary && (
                      <p className="text-sm text-tastelanc-text-muted mb-2">{job.compensation_summary}</p>
                    )}
                    ...
                    {isExpanded && (
                      <div className="mt-3 text-sm text-tastelanc-text-secondary whitespace-pre-wrap border-t border-tastelanc-surface-light pt-3">
                        {job.description}
                        {job.requirements && job.requirements.length > 0 && (
                          <div className="mt-3">
                            <p className="font-medium text-tastelanc-text-primary mb-1">Requirements:</p>
