...
interface UnifiedCheckoutWizardProps {
  apiEndpoint: string; // '/api/checkout/multi-restaurant'
  successUrl: string; // Where to redirect after checkout success
  cancelUrl: string; // Where to redirect if cancelled
  restaurantsApiUrl: string; // '/api/sales/restaurants' or '/api/admin/restaurants'
  storageKey: string; // 'salesCheckoutWizard' or 'adminCheckoutWizard'
  pageTitle?: string;
...
export function UnifiedCheckoutWizard({
  apiEndpoint,
  successUrl,
  cancelUrl,
  restaurantsApiUrl,
  storageKey,
  pageTitle = 'New Sale',
  pageDescription = 'Build a checkout link for one or more restaurants',
}: UnifiedCheckoutWizardProps) {
  ...
  }, [step, email, contactName, phone, cart, currentPlan, currentDuration, checkoutUrl, storageKey]);
  ...
  useEffect(() => {
    const fetchRestaurants = async () => {
      try {
        ...
        while (hasMore) {
          const res = await fetch(`${restaurantsApiUrl}?limit=${PAGE_SIZE}&page=${page}`, { credentials: 'include' });
          if (!res.ok) break;
  ...
  const handleNextStep = () => {
    setStepError('');
    if (step === 1) {
      if (!email) { setStepError('Customer email is required'); return; }
      setStep(2);
    } else if (step === 2) {
      if (cart.length === 0) { setStepError('Add at least one restaurant'); return; }
      setStep(3);
    }
  };
  ...
  const resetForm = () => {
    setEmail(''); setContactName(''); setPhone(''); setCart([]);
    ...
    <div className="max-w-xl mx-auto">
      <div className="mb-8">
        <div className="flex items-center gap-3">
          <h1 className="text-2xl font-bold text-tastelanc-text-primary">{pageTitle}</h1>
          <Tooltip content={pageDescription} position="bottom">
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-3">
                ...
                <button onClick={handleCreateCheckout} disabled={isProcessing}
                  ...
                  {isProcessing ? <><Loader2 className="w-4 h-4 animate-spin" /> Creating...</> : <><CreditCard className="w-4 h-4" /> Create Checkout</>}
                </button>
              </div>
            </div>
          )}
