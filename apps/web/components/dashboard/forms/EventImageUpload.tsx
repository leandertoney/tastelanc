...
export default function EventImageUpload({ value, onChange, restaurantId }: EventImageUploadProps) {
  ...
  const handleUpload = useCallback(async (file: File) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/dashboard/events/upload?restaurant_id=${restaurantId}`,
        {
          method: 'POST',
          body: formData,
        }
  ...
  if (value) {
    return (
      <div className="relative">
        ...
        <p className="text-xs text-tastelanc-text-faint mt-1.5">Custom artwork uploaded</p>
      </div>
    );
  }
          ...
          <>
            ...
            <div className="text-center">
              <span className="text-sm text-tastelanc-text-secondary">
                {isDragOver ? 'Drop image here' : 'Add custom artwork'}
              </span>
