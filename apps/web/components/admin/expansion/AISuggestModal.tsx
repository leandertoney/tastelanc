      ...
      <div className="relative bg-tastelanc-surface rounded-xl border border-tastelanc-surface-light w-full max-w-2xl shadow-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between p-5 border-b border-tastelanc-surface-light flex-shrink-0">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold text-tastelanc-text-primary">AI City Suggestions</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleGenerate} className="p-5 border-b border-tastelanc-surface-light flex-shrink-0">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div>
              <label className="block text-xs font-medium text-tastelanc-text-muted mb-1">State</label>
              <input
                type="text"
                value={state}
                onChange={(e) => setState(e.target.value)}
                placeholder="PA"
                className="w-full px-3 py-2 bg-tastelanc-surface-light border border-tastelanc-surface-light rounded-lg text-tastelanc-text-primary placeholder-tastelanc-text-faint focus:outline-none focus:ring-2 focus:ring-tastelanc-accent text-sm"
              />
            </div>
            <div>
              <label className="block text-xs font-medium text-tastelanc-text-muted mb-1">Min Population</label>
              <input
                type="number"
                value={minPopulation}
            ...
            <div>
              <label className="block text-xs font-medium text-tastelanc-text-muted mb-1">Max Population</label>
              <input
                type="number"
                value={maxPopulation}
            ...
            <div>
              <label className="block text-xs font-medium text-tastelanc-text-muted mb-1">Count</label>
              <input
                type="number"
                value={count}
        ...
        <div className="flex-1 overflow-y-auto p-5">
          {isGenerating && (
            <div className="flex flex-col items-center justify-center py-12 text-tastelanc-text-muted">
              <Loader2 className="w-8 h-8 animate-spin text-lancaster-gold mb-3" />
              <p className="text-sm">AI is analyzing potential markets...</p>
              <p className="text-xs text-tastelanc-text-faint mt-1">This may take 15-30 seconds</p>
            </div>
          )}

          {!isGenerating && suggestions.length === 0 && (
            <div className="flex flex-col items-center justify-center py-12 text-tastelanc-text-faint">
              <Sparkles className="w-10 h-10 mb-3 text-tastelanc-text-faint" />
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <h4 className="font-semibold text-tastelanc-text-primary">{suggestion.city_name}</h4>
                          <span className="text-tastelanc-text-faint text-sm">{suggestion.state}</span>
                        </div>

                        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-tastelanc-text-muted mb-2">
                        ...
                        <p className="text-xs text-tastelanc-text-faint leading-relaxed">
