/**
 * TasteFayetteville Restaurant Import Script
 *
 * Imports restaurants from the Dreamville CSV scrape into Supabase.
 *
 * Logic:
 ...
 *  4. Import to `restaurants` table with market_id = fayetteville-nc
 *  5. Import contacts to `business_leads` table (one row per contact)
 *
 * Usage:
 *   cd apps/web
 *   SUPABASE_SERVICE_ROLE_KEY=<key> CSV_PATH=<path> npx tsx scripts/import-fayetteville-restaurants.ts
...
async function main() {
  console.log(`\n🌟 TasteFayetteville Restaurant Import`);
  ...
  for (const r of finalRestaurants) {
    ...
    let base = slugify(r.name) + '-fayetteville';
    let slug = base;
    let counter = 2;
...
function writeReport(stats: ImportStats, restaurants: RestaurantRecord[]) {
  ...
  const reportPath = path.join(process.cwd(), 'fayetteville-import-report.json');
