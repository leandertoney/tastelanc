...
import { ScoredCandidate, ContentType } from './types';
...
export function selectTopCandidates(
  ...
  return {
    visible: deduped.slice(0, visibleCount),
...
export async function fetchTonightCandidates(
  ...
  const { data: events } = await supabase
    .from('events')
    .select(`
      id, name, event_type, image_url, created_at, is_recurring, days_of_week, event_date,
  ...
  const scoredEvents = (events || [])
    ...
    .map((e: any) =>
      scoreCandidate({
        ...
        tier_slug: e.restaurant.tier?.slug || null,
  ...
  const { data: happyHours } = await supabase
    .from('happy_hours')
    .select(`
      id, name, description, image_url, created_at, days_of_week,
      restaurant:restaurants!inner(id, name, slug, cover_image_url, average_rating, market_id, is_active,
        tier:tiers(slug)
      )
    `)
  ...
  const scoredHappyHours = (happyHours || [])
    ...
    .map((h: any) =>
      scoreCandidate({
        ...
        tier_slug: h.restaurant.tier?.slug || null,
  ...
  const scoredSpecials = (specials || [])
    ...
    .map((s: any) =>
      scoreCandidate({
        ...
        tier_slug: s.restaurant.tier?.slug || null,
