...
export function useRestaurants(options: UseRestaurantsOptions = {}) {
  ...
  return useQuery({
    queryKey: [...queryKeys.restaurants.list(category === 'all' ? undefined : category), marketId],
 ...
 * (No market filter — detail view is by ID, market-agnostic)
 */
...
export function useRestaurantSearch(
  ...
  return useQuery({
    queryKey: [...queryKeys.restaurants.search(query, category), marketId],
    queryFn: async (): Promise<Restaurant[]> => {
      ...
      if (marketId) {
        supabaseQuery = supabaseQuery.eq('market_id', marketId);
      }
 ...
 * (No market filter — detail view is by ID)
 */
