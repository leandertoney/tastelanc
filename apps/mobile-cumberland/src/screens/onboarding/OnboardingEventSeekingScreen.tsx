...
export default function OnboardingEventSeekingScreen({ navigation }: Props) {
  ...
  const handleToggle = (option: string) => {
    ...
    if (isCurrentlySelected || data.eventPreferences.length < MAX_SELECTIONS) {
      toggleEventPreference(option);
    }
  };
      ...
      <View style={styles.content}>
        <Animated.View style={[styles.headerSection, titleAnimatedStyle]}>
          ...
          <Text style={styles.subheadline}>
            Pick up to 3 ({data.eventPreferences.length}/3 selected)
          </Text>
        </Animated.View>

        <Animated.View style={[styles.gridContainer, contentAnimatedStyle]}>
          <MultiSelectGrid
            options={EVENT_GRID_OPTIONS}
            selected={data.eventPreferences}
