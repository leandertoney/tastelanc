// Welcome
export { default as OnboardingSlidesScreen } from './OnboardingSlidesScreen';

// Problem & Solution
export { default as OnboardingProblemsScreen } from './OnboardingProblemsScreen';
export { default as OnboardingSolutionScreen } from './OnboardingSolutionScreen';

// Feature Discovery (animated)
export { default as OnboardingHappyHoursScreen } from './OnboardingHappyHoursScreen';
export { default as OnboardingEventsScreen } from './OnboardingEventsScreen';
export { default as OnboardingSpecialsScreen } from './OnboardingSpecialsScreen';

// Personal Questions (auto-advance on single select)
export { default as OnboardingUserTypeScreen } from './OnboardingUserTypeScreen';
export { default as OnboardingNameScreen } from './OnboardingNameScreen';
export { default as OnboardingDiningHabitsScreen } from './OnboardingDiningHabitsScreen';
export { default as OnboardingEventSeekingScreen } from './OnboardingEventSeekingScreen';

// Preferences (multi-select with Continue button)
export { default as OnboardingBudgetScreen } from './OnboardingBudgetScreen';
export { default as OnboardingEntertainmentScreen } from './OnboardingEntertainmentScreen';
export { default as OnboardingFoodScreen } from './OnboardingFoodScreen';

// Summary & Conversion
export { default as OnboardingPremiumScreen } from './OnboardingPremiumScreen';
export { default as OnboardingMollieAskScreen } from './OnboardingMollieAskScreen';
export { default as OnboardingVotingScreen } from './OnboardingVotingScreen';
export { default as OnboardingReviewAskScreen } from './OnboardingReviewAskScreen';
export { default as OnboardingPremiumIntroScreen } from './OnboardingPremiumIntroScreen';

// Legacy (backward compatibility)
export { default as OnboardingFrequencyScreen } from './OnboardingFrequencyScreen';
export { default as OnboardingDiscoveryScreen } from './OnboardingDiscoveryScreen';
export { default as OnboardingPreferencesScreen } from './OnboardingPreferencesScreen';
