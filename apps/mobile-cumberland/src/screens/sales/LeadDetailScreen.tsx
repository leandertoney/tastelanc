...
export default function LeadDetailScreen() {
  ...
  const handleEmail = useCallback(() => {
    const email = data?.lead?.contact_email || data?.lead?.email;
    if (!email) return;
  ...
  const handleCall = useCallback(() => {
    const phone = data?.lead?.contact_phone || data?.lead?.phone;
    if (!phone) return;
    Linking.openURL(`tel:${phone}`);
  }, [data?.lead?.contact_phone, data?.lead?.phone]);
    ...
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.card}>
          ...
          {/* Direct Contact (personal info from CSV enrichment) */}
          {(lead.contact_phone || lead.contact_email) && (
            <View style={styles.directContactSection}>
              <Text style={styles.directContactLabel}>Direct Contact</Text>
              {lead.contact_phone && (
                <TouchableOpacity
                ...
                >
                  ...
                  <Text style={[styles.infoText, { color: colors.success }]}>{lead.contact_phone}</Text>
                </TouchableOpacity>
              )}
              {lead.contact_email && (
                <TouchableOpacity
                  ...
                  onPress={() => navigation.navigate('ComposeEmail', {
                    ...
                    recipientName: lead.contact_name || lead.business_name,
                  })}
                >
                  <Ionicons name="person-outline" size={16} color={colors.accent} />
                  <Text style={[styles.infoText, { color: colors.accent }]}>{lead.contact_email}</Text>
                </TouchableOpacity>
              )}
            </View>
          )}
        ...
        <View style={styles.actions}>
          {(() => {
            const primaryEmail = lead.contact_email || lead.email;
            const primaryPhone = lead.contact_phone || lead.phone;
            return (
              <>
                <TouchableOpacity
                  style={[styles.actionButton, !primaryEmail && styles.actionDisabled]}
                  onPress={handleEmail}
                  disabled={!primaryEmail}
                >
                  <Ionicons name="mail" size={20} color={primaryEmail ? colors.accent : colors.textSecondary} />
                  <Text style={[styles.actionLabel, !primaryEmail && styles.actionLabelDisabled]}>Email</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={[styles.actionButton, !primaryPhone && styles.actionDisabled]}
                  onPress={handleCall}
                  disabled={!primaryPhone}
                >
                  <Ionicons name="call" size={20} color={primaryPhone ? colors.success : colors.textSecondary} />
                  <Text style={[styles.actionLabel, !primaryPhone && styles.actionLabelDisabled]}>Call</Text>
                </TouchableOpacity>
              </>
            );
          })()}
