...
export default function HomeScreen() {
  ...
  const [mollieChatVisible, setMollieChatVisible] = useState(false);

  const { data: favorites = EMPTY_FAVORITES } = useFavorites();
  ...
  const { data: featuredRestaurants = EMPTY_RESTAURANTS } = useQuery({
    queryKey: ['featuredRestaurants', marketId],
    queryFn: () => getFeaturedRestaurants(16, marketId),
    staleTime: 5 * 60 * 1000,
    enabled: !!marketId, // Don't fetch until market is resolved
  });
  ...
  const {
    ...
    initialPageParam: 0,
    enabled: !!marketId, // Wait until market is resolved
    staleTime: 5 * 60 * 1000,
  });
  ...
  return (
    <GestureHandlerRootView style={styles.gestureRoot}>
      <SafeAreaView style={styles.container} edges={['bottom']}>
        ...
        {/* Mollie AI FAB */}
        <TouchableOpacity
          style={styles.mollieFab}
          onPress={() => setMollieChatVisible(true)}
          activeOpacity={0.9}
        >
          <Image source={mollieImage} style={styles.mollieAvatar} />
          <Text style={styles.mollieFabText}>{`Ask ${BRAND.aiName}`}</Text>
        </TouchableOpacity>

        {/* Mollie Chat Bottom Sheet */}
        <MollieChat
          visible={mollieChatVisible}
          onClose={() => setMollieChatVisible(false)}
