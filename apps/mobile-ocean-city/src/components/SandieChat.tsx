...
// Sandie assets
const sandieImage = require('../../assets/images/sandie_avatar.png');
const sandieAnimated = require('../../assets/animations/sandie_animated.mp4');
...
export default function SandieChat({ visible, onClose, onNavigateToRestaurant }: SandieChatProps) {
  const bottomSheetRef = useRef<BottomSheet>(null);
  const flatListRef = useRef<any>(null);

  const sandiePlayer = useVideoPlayer(sandieAnimated, player => {
    player.loop = true;
    player.muted = true;
  });
  ...
  const sendMessage = useCallback(async () => {
    ...
    try {
      ...
      console.error('Sandie chat error detail:', errorDetail);
  ...
  const renderMessageText = (text: string, isUser: boolean) => {
    if (isUser) {
      return <Text style={[styles.messageText, styles.userMessageText]}>{text}</Text>;
    }
    ...
    >
      {!item.isUser && (
        <View style={styles.avatarContainer}>
          <Image source={sandieImage} style={styles.sandieAvatar} />
        </View>
      )}
  ...
  const handleQuickAction = useCallback(async (text: string) => {
    ...
    try {
      ...
      console.error('Sandie chat error:', error);
      const errorMessage: Message = {
        id: (Date.now() + 1).toString(),
        text: "I'm having a little trouble connecting right now. Please try again in a moment!",
        isUser: false,
  ...
  return (
    ...
    >
      <View style={styles.sandieContainer}>
        {/* Header */}
        <View style={styles.header}>
          <View style={styles.headerLeft}>
            <View style={styles.sandieHeaderAvatar}>
              <VideoView
                player={sandiePlayer}
                style={styles.sandieHeaderImage}
                contentFit="cover"
...
const styles = StyleSheet.create({
  ...
  sandieHeaderImage: { width: 48, height: 48 },
  ...
  sandieAvatar: { width: 32, height: 32, borderRadius: 16 },
  ...
  restaurantLink: { color: colors.accent, fontWeight: '600', textDecorationLine: 'underline' },
  ...
  quickActionsTitle: { fontSize: 12, color: colors.textMuted, marginBottom: 8 },
  ...
  quickActionText: { fontSize: 13, color: colors.text, fontWeight: '500', flex: 1 },
  inputContainer: {
    flexDirection: 'row',
    alignItems: 'flex-end',
    paddingHorizontal: 16,
  ...
  sendButtonDisabled: { backgroundColor: colors.cardBgElevated },
});
