...
def draw_rosie_fab(draw, fonts, y_offset=0):
    """Draw the Ask Rosie floating action button"""
    fab_width = 280
    fab_height = 100
    ...
    # Text
    draw.text((fab_x + 100, fab_y + 30), "Ask Rosie",
              font=fonts['body'], fill=hex_to_rgb(COLORS['text']))
...
def create_rosie_screen(output_dir, fonts):
    """Create Rosie AI chat mockup"""
    img = Image.new('RGB', (WIDTH, HEIGHT), hex_to_rgb(COLORS['primary']))
    draw = ImageDraw.Draw(img)
    ...
    draw.text((WIDTH // 2 - 100, header_y + 36), "Chat with Rosie",
    ...
    draw_rounded_rect(draw,
        ...
        radius=RADIUS['lg'],
        fill=hex_to_rgb(COLORS['accent']))
    draw.text((60, y + 24), "Hi! I'm Rosie, your food concierge.",
              font=fonts['body'], fill=hex_to_rgb(COLORS['text']))
    ...
    draw_rounded_rect(draw,
        ...
        radius=40,
        fill=hex_to_rgb(COLORS['cardBg']))
    draw.text((72, input_y + 56), "Ask Rosie anything...",
              font=fonts['body'], fill=hex_to_rgb(COLORS['textSecondary']))
    ...
    img.save(os.path.join(output_dir, '04_rosie.png'), 'PNG')
    print("Created: 04_rosie.png")
...
def main():
    ...
    create_rosie_screen(output_dir, fonts)
