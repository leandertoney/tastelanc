...
function ConversationRow({ item, onPress }: { item: Conversation; onPress: () => void }) {
  ...
  const initial = (item.counterparty_name || item.counterparty_email)[0].toUpperCase();
      ...
      {isLoading && !data ? (
        ...
        <FlatList
          ...
          contentContainerStyle={
            (data?.conversations?.length ?? 0) === 0 ? styles.emptyContainer : undefined
          }
          ListEmptyComponent={
