...
async function getAllHappyHours(marketId: string | null = null): Promise<HappyHourWithRestaurant[]> {
...
export default function HappyHoursViewAllScreen() {
  ...
  const { data: happyHours = [], isLoading, refetch, isRefetching } = useQuery({
    queryKey: ['allHappyHours', marketId],
    queryFn: () => getAllHappyHours(marketId),
    staleTime: 5 * 60 * 1000,
  });
  ...
  const renderItem = ({ item }: { item: HappyHourWithRestaurant }) => {
    ...
    return (
      <SpotifyStyleListItem
        ...
        accentText={dealText}
        subtitle={`${item.description || item.name} · ${formatTimeWindow(item.start_time, item.end_time)} · ${formatDays(item.days_of_week)}`}
