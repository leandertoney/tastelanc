...
import { fetchEntertainmentEvents, fetchEvents, ApiEvent } from './events';
...
function getVoterParticipationText(voterCount: number): string {
  ...
  if (voterCount < 10) return 'Join Lancaster locals in voting';
  ...
  return 'Thousands of Lancaster locals voting';
}
...
async function getPlatformSocialProof() {
  ...
  if (error || !data) {
    ...
    return {
      voterParticipation: 'Join Lancaster locals in voting',
      categoriesActive: 8,
      ...
      communityText: 'Join the Lancaster dining community',
    };
  }
  ...
  return {
    ...
    communityText: stats?.voters_this_month > 10
      ...
      : 'Join the Lancaster dining community',
  };
}
