...
CREATE INDEX IF NOT EXISTS idx_ig_memory_market_restaurant ON public.instagram_post_memory(market_id, restaurant_id);
CREATE INDEX IF NOT EXISTS idx_ig_memory_market_asset ON public.instagram_post_memory(market_id, asset_url);
...
CREATE INDEX IF NOT EXISTS idx_ig_logs_market_date ON public.instagram_generation_logs(market_id, run_at DESC);
CREATE INDEX IF NOT EXISTS idx_ig_posts_market_date ON public.instagram_posts(market_id, post_date DESC);
CREATE INDEX IF NOT EXISTS idx_ig_posts_status ON public.instagram_posts(status);
...
SELECT cron.schedule(
  ...
  '0 4 * * *',
  $$SELECT net.http_post(
    url := 'https://tastelanc.com/api/instagram/refresh-token',
...
SELECT cron.schedule(
  ...
  '0 5 * * *',
  $$SELECT net.http_post(
    url := 'https://tastelanc.com/api/instagram/metrics',
    body := '{"source": "pg_cron"}'::jsonb,
    headers := jsonb_build_object(
      'Content-Type', 'application/json',
      'Authorization', 'Bearer 9e98786de33d85396badebb998129d54f879970287be03f1ed5704a8663e6912'
